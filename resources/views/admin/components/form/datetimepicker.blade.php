@if (!isset($attributes['no-label']))
    ...
    @endif
@endif
{{ Form::text($name, old($name) ?? $value, array_merge(['id'=>$name,isset($syncedAttributes) && in_array($name, $syncedAttributes) ? 'disabled' : '','class' => 'form-control form-control-lg mb-3 ' . ($errors->has($name) ? 'is-invalid' : '')])) }}
@if($errors->has($name))
