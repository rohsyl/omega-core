@php
    $labelPrefix = 'label.acl_.';
    if(isset($attributes['label_prefix'])) {
        $labelPrefix = $attributes['label_prefix'];
    }
@endphp
<li class="list-group-item @if(isset($attributes['dark']) && $attributes['dark']) bg-primary-darker @endif px-0 py-0 border-0"  data-level="{{ $level }}">
    ...
    <span style="padding-left: {{ 20 * ($level + 1) }}px" class="my-1 pb-1 d-block @if(isset($attributes['dark']) && $attributes['dark']) border-bottom-secondary @else border-bottom @endif">
        ...
        {{ Form::label($permissionName, __($labelPrefix.str_replace('.', '_', $permissionName)), ['class' => 'm-0']) }}
    </span>
    <script>
    ...
    @if(sizeof($subPermissions) > 0)
        ...
        @foreach($subPermissions as $subPermissionName => $subSubPermissions)
            @include('omega::admin.components.acl.item', ['name' => $name, 'permissionName' => $permissionName.'.'.$subPermissionName, 'subPermissions' => $subSubPermissions, 'user' => $user, 'level' => $level, 'attributes' => $attributes])
        @endforeach
        </ul>
    @endif
</li>
